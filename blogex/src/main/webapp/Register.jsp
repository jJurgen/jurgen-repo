	...
	<div id="container">
		...
		<div id="content">			
			<form id="regForm" action="RegServlet" method="post">
				<div class="field">
					<label>E-mail:*<input type="text" name="inputTextEmail" maxlength="35"></label>
				</div>
				<c:choose>
					<c:when test="${not empty invalidData.emailMessage}">
						<label class="errorMessage"><c:out value="${invalidData.emailMessage}"/></label>	
					</c:when>
					<c:when test="${not empty requestScope.emailUnique}">
						<label class="errorMessage"><c:out value="${requestScope.emailUnique}"/></label>	
					</c:when>
					<c:otherwise>
				...
				<div class="field">
					<label>Nickname:*<input type="text" name="inputTextNickname" maxlength="35"></label>
				</div>
				<c:choose>
					<c:when test="${not empty invalidData.nicknameMessage}">
					<label class="errorMessage"><c:out value="${invalidData.nicknameMessage}"/></label>	
					</c:when>
					<c:when test="${not empty requestScope.nicknameUnique}">
						<label class="errorMessage"><c:out value="${requestScope.nicknameUnique}"/></label>	
					</c:when>
					<c:otherwise>
						<label class="errorMessage"></label>
					</c:otherwise>
				...
				<div class="field">
					<label>Password:*<input type="password" name="inputTextPassword" maxlength="35"></label>
				</div>
				<label class="errorMessage"><c:out value="${invalidData.passwordMessage}"/></label>
				<div class="field">
					<label>Repeat Password:*<input type="password" name="inputTextRepeatedPassword" maxlength="35"></label>
				</div>
				<label class="errorMessage"><c:out value="${invalidData.passwordsEqualityMessage}"/></label>				
				<button type="submit">Create account</button>
								
			</form>
				<h3>
					<c:out value="${requestScope.userRegAttr}" />
				</h3>			
		</div>
