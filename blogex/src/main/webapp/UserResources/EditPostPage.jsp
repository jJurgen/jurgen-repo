	...
	<div id="container">
		...
		<div id="content">
			<div id="blogContainer">				
				<c:choose>
					<c:when test="${not empty sessionScope.currentUser}">
						<c:if test="${not empty sessionScope.editingPost}">							
							<c:if test="${sessionScope.currentUser.nickname eq sessionScope.editingPost.author}">													
								<form id="writingNewPostForm" action="../EditPostServlet" method="POST">
									<label class="title">Change something</label>
									<label class="inputTitle">Title:</label>
									<textarea class="titleTextArea" name="titleArea" maxlength="100"><c:out value="${sessionScope.editingPost.title}"/></textarea>
									<label class="inputTitle">Text:</label>
									<textarea class="contentTextArea" name="contentArea" maxlength="10000"><c:out value="${sessionScope.editingPost.content}"/></textarea>
					...
					<c:otherwise>
						<label class="title">You aren't logged in</label>
					</c:otherwise>	
				</c:choose>				
			</div>
		</div>
